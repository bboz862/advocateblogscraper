...
Player.Playlist = function() {
    ...
    */
    function Playlist(player, playing_from, referer, options) {
        ...
        this._ui_listeners = new Object(); //whoever instantiates the 
                                            //playlist may want to listen for certain events.
    ...
    Playlist.prototype = {
        ...
        update_trackinfo : function(i, trackinfo) {
            ...
            {
                if(this.first_playable_track == -1 ||
                    i < this.first_playable_track)
                {
                    this.first_playable_track = i;
                }
                if(this.last_playable_track == -1 ||
                    i > this.last_playable_track)
                {
        ...
        play : function() {
            switch(this._state)
            {
                ...
                case "BUSY":
                    ...
                    while(!tinfo.is_playable() &&
                        this._track < this.last_playable_track)
                    {
                        this._track++;
                    ...
                    {
                        // fixme: this handles the case where we are
                        // sitting, stopped, at the end of the last
                        // track, and someone hits play.  I think it
                        // would be cleaner if we just were not
                        // allowed to sit in that state (but instead
                        // were placed at the beginning of the first
                        // track).  Clean that up.
                        ...
                        if(this._playlist[this._track].file)
                        {
                            this._load();
                            this._player.play();
                            if(this._usecomm) {
        ...
        register_listener : function( event_name, listener ) {  
            ...
            if ( evlisteners ){
                $assert( Y.lang.isArray( evlisteners ), "expected array of event listeners" );
...
Player.PlayStat = function() {
    ...
    PlayStat.prototype.play_progress = function(secs_played, secs_total) {
        ...
        // be called any more.  (returns false once "completed" is sent)
            ...
            var partial = secs_total * SKIP_WINDOW / 100;
            var complete = secs_total * (100 - COMPLETE_WINDOW) / 100;

            if(!this._partial_sent && secs_played > partial)
            {
                // Log.info("partial play!");
                this._partial_sent = true;
                this.change_phase("partial");
            }
            if(!this._complete_sent && secs_played > complete)
            {
